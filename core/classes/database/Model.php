...
class Model
{
    ...
    public function exportToCsv($name, $conditions = [])
    {
        $sql_array = $this->load($conditions);

        $filename = "members_" . date('Y-m-d') . ".csv";
        $delimiter = ",";
        $f = fopen($filename, 'r+');
        fputcsv($f, ['code','jarCode', 'name', 'municipality', 'ecoActCode', 'ecoActName', 'month', 'avgWage', 'numInsured', 'avgWage2', 'numInsured2', 'tax'], $delimiter);
        foreach ($sql_array as $data) {
